...
import { uploadFeedPhotoToStorage, addNewPost } from '../functions/post'
...
const PostFeed = props => {
  ...
  const [videoURL, setVideoURL] = useState('https://www.youtube.com/watch?v=9R7Mp2NeAeQ')
  const [videoInfoImageURL, setVideoInfoImageURL] = useState('')
  ...
  const handleAddNewPost = async () => {
    const postId = thisPostId ? thisPostId : uuidv4();
    console.log({linkURL})
    const result = await addNewPost(inputText, photoUploaded ? true : false, linkURL, postId) 
      .then((data) => {
        console.log('HERE', {data})
        data.data.createPost['createdAt'] = new Date()
        props.navigation.navigate('Feed', {newLocalPost: data.data.createPost})
        return data
      })
      .catch((err) => {
  ...
  const handleUploadPhoto = async () => {
    const resultPickImage = await _pickImage();
    setLoadingUploadPhoto(true)
    const uuid = await uuidv4()
    setThisPostId(uuid);
    const result = await uploadFeedPhotoToStorage(resultPickImage, uuid).then(async (data) => {
